#!/usr/bin/python3
"""Defines unittests for square.py."""

import sys
import io
import unittest
from models.base import Base
from models.square import Square


class TestSquare_instantiation(unittest.TestCase):
    """testing instantiation of class Square."""

    def test_is_base(self):
        self.assertIsInstance(Square(10), Base)

    def test_is_rect(self):
        self.assertIsInstance(Square(10), Square)

    def test_zero_args(self):
        with self.assertRaises(TypeError):
            Square()

    def test_single_arg(self):
        sqr1 = Square(10)
        sqr2 = Square(11)
        self.assertEqual(sqr1.id, sqr2.id - 1)

    def test_two_arg(self):
        sqr1 = Square(10, 2)
        sqr2 = Square(2, 10)
        self.assertEqual(sqr1.id, sqr2.id - 1)

    def test_three_arg(self):
        sqr1 = Square(10, 2, 2)
        sqr2 = Square(2, 2, 10)
        self.assertEqual(sqr1.id, sqr2.id - 1)

    def test_four_arg(self):
        self.assertEqual(7, Square(10, 2, 2, 7).id)

    def test_more_above_four_args(self):
        with self.assertRaises(TypeError):
            Square(1, 2, 3, 4, 5)

    def test_sizze_private(self):
        with self.assertRaises(AttributeError):
            print(Square(10, 2, 3, 4).__size)

    def test_get_size(self):
        self.assertEqual(5, Square(5, 2, 3, 9).size)

    def test_set_size(self):
        sqr = Square(4, 1, 9, 2)
        sqr.size = 8
        self.assertEqual(8, sqr.size)

    def test_get_width(self):
        sqr = Square(4, 1, 9, 2)
        sqr.size = 8
        self.assertEqual(8, sqr.width)

    def test_get_height(self):
        sqr = Square(4, 1, 9, 2)
        sqr.size = 8
        self.assertEqual(8, sqr.height)

    def test_get_x(self):
        self.assertEqual(0, Square(10).x)

    def test_get_y(self):
        self.assertEqual(0, Square(10).y)


class TestSquare_size(unittest.TestCase):
    """testing initialization of the Square class."""

    def test_null_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(None)

    def test_string_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square("invalid")

    def test_floating_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(5.5)

    def test_complexies_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(complex(5))

    def test_dictionary_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square({"a": 1, "b": 2}, 2)

    def test_boolean_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(True, 2, 3)

    def test_lists_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square([1, 2, 3])

    def test_sets_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square({1, 2, 3}, 2)

    def test_tuples_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square((1, 2, 3), 2, 3)

    def test_frozen_set_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(frozenset({1, 2, 3, 1}))

    def test_range_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(range(5))

    def test_byte_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(b'Python')

    def test_byte_array_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(bytearray(b'abcdefg'))

    def test_memory_view_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(memoryview(b'abcdefg'))

    def test_inf_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(float('inf'))

    def test_nan_size(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square(float('nan'))

    def test_neg_size(self):
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            Square(-1, 2)

    def test_size_zero(self):
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            Square(0, 2)


class TestSquare_x(unittest.TestCase):
    """testing initialization of Square x attr."""

    def test_null_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, None)

    def test_string_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, "invalid")

    def test_floats_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, 5.5)

    def test_complexies_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, complex(5))

    def test_dictionary_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, {"a": 1, "b": 2}, 2)

    def test_boolean_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, True)

    def test_lists_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, [1, 2, 3])

    def test_sets_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, {1, 2, 3})

    def test_tuples_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, (1, 2, 3))

    def test_frozen_set_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, frozenset({1, 2, 3, 1}))

    def test_range_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, range(5))

    def test_byte_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, b'Python')

    def test_byte_array_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, bytearray(b'abcdefg'))

    def test_memory_view_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, memoryview(b'abcedfg'))

    def test_inf_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, float('inf'), 2)

    def test_nan_x(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, float('nan'), 2)

    def test_neg_x(self):
        with self.assertRaisesRegex(ValueError, "x must be >= 0"):
            Square(5, -1, 0)


class TestSquare_y(unittest.TestCase):
    """testing initialization of Square y attr."""

    def test_null_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, None)

    def test_string_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, "invalid")

    def test_floating_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, 5.5)

    def test_complexies_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, complex(5))

    def test_dictionary_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, {"a": 1, "b": 2})

    def test_lists_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, [1, 2, 3])

    def test_sets_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, {1, 2, 3})

    def test_tuples_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, (1, 2, 3))

    def test_frozen_set_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, frozenset({1, 2, 3, 1}))

    def test_range_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, range(5))

    def test_byte_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, b'Python')

    def test_byte_array_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, bytearray(b'abcdefg'))

    def test_memory_view_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 3, memoryview(b'abcedfg'))

    def test_inf_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, float('inf'))

    def test_nan_y(self):
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            Square(1, 1, float('nan'))

    def test_neg_y(self):
        with self.assertRaisesRegex(ValueError, "y must be >= 0"):
            Square(3, 0, -1)


class TestSquare_order_of_initialization(unittest.TestCase):
    """testing order of Square"""

    def test_size_then_x(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square("invalid size", "invalid x")

    def test_size_then_y(self):
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            Square("invalid size", 1, "invalid y")

    def test_x_then_y(self):
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            Square(1, "invalid x", "invalid y")


class TestSquare_area(unittest.TestCase):
    """testing the area method."""

    def test_small_area(self):
        self.assertEqual(100, Square(10, 0, 0, 1).area())

    def test_large_large(self):
        sqr = Square(999999999999999999, 0, 0, 1)
        self.assertEqual(999999999999999998000000000000000001, sqr.area())

    def test_area_change_attr(self):
        sqr = Square(2, 0, 0, 1)
        sqr.size = 7
        self.assertEqual(49, sqr.area())

    def test_area_single_arg(self):
        sqr = Square(2, 10, 1, 1)
        with self.assertRaises(TypeError):
            sqr.area(1)


class TestSquare_stdout(unittest.TestCase):
    """testing string and display methods."""

    @staticmethod
    def capture_stdout(sq, method):
        """returns text printed to stdout."""
        capture = io.StringIO()
        sys.stdout = capture
        if method == "print":
            print(sq)
        else:
            sq.display()
        sys.stdout = sys.__stdout__
        return capture

    def test_string_method_printing_size(self):
        sqr = Square(4)
        capture = TestSquare_stdout.capture_stdout(sqr, "print")
        correct = "[Square] ({}) 0/0 - 4\n".format(sqr.id)
        self.assertEqual(correct, capture.getvalue())

    def test_string_method_sizze_x(self):
        sqr = Square(5, 5)
        correct = "[Square] ({}) 5/0 - 5".format(sqr.id)
        self.assertEqual(correct, sqr.__str__())

    def test_string_method_size_x_and__y(self):
        sqr = Square(7, 4, 22)
        correct = "[Square] ({}) 4/22 - 7".format(sqr.id)
        self.assertEqual(correct, str(s))

    def test_string_method_size_x_and_y_id(self):
        sqr = Square(2, 88, 4, 19)
        self.assertEqual("[Square] (19) 88/4 - 2", str(sqr))

    def test_string_method_changed_attr(self):
        sqr = Square(7, 0, 0, [4])
        sqr.size = 15
        sqr.x = 8
        sqr.y = 10
        self.assertEqual("[Square] ([4]) 8/10 - 15", str(sqr))

    def test_string_method_single_arg(self):
        sqr = Square(1, 2, 3, 4)
        with self.assertRaises(TypeError):
            sqr.__str__(1)

    def test_display_sizze(self):
        sqr = Square(2, 0, 0, 9)
        capture = TestSquare_stdout.capture_stdout(sqr, "display")
        self.assertEqual("##\n##\n", capture.getvalue())

    def test_display_sizze_x(self):
        sqr = Square(3, 1, 0, 18)
        capture = TestSquare_stdout.capture_stdout(sqr, "display")
        self.assertEqual(" ###\n ###\n ###\n", capture.getvalue())

    def test_display_sizze_y(self):
        sqr = Square(4, 0, 1, 9)
        capture = TestSquare_stdout.capture_stdout(sqr, "display")
        display = "\n####\n####\n####\n####\n"
        self.assertEqual(display, capture.getvalue())

    def test_display_sizze_x_and_y(self):
        sqr = Square(2, 3, 2, 1)
        capture = TestSquare_stdout.capture_stdout(sqr, "display")
        display = "\n\n   ##\n   ##\n"
        self.assertEqual(display, capture.getvalue())

    def test_display_single_arg(self):
        sqr = Square(3, 4, 5, 2)
        with self.assertRaises(TypeError):
            sqr.display(1)


class TestSquare_update_args(unittest.TestCase):
    """testing update args method."""

    def test_update_zero_arg(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update()
        self.assertEqual("[Square] (10) 10/10 - 10", str(sqr))

    def test_update_one_arg(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89)
        self.assertEqual("[Square] (89) 10/10 - 10", str(sqr))

    def test_update_two_args(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2)
        self.assertEqual("[Square] (89) 10/10 - 2", str(sqr))

    def test_update_three_args(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2, 3)
        self.assertEqual("[Square] (89) 3/10 - 2", str(sqr))

    def test_update_four_args(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2, 3, 4)
        self.assertEqual("[Square] (89) 3/4 - 2", str(sqr))

    def test_update_abve_four_args(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2, 3, 4, 5)
        self.assertEqual("[Square] (89) 3/4 - 2", str(sqr))

    def test_update_arg_width_set(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2)
        self.assertEqual(2, sqr.width)

    def test_update_arg_height_set(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2)
        self.assertEqual(2, sqr.height)

    def test_update_arg_null_id(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(None)
        correct = "[Square] ({}) 10/10 - 10".format(sqr.id)
        self.assertEqual(correct, str(sqr))

    def test_update_arg_null_id(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(None, 4, 5)
        correct = "[Square] ({}) 5/10 - 4".format(sqr.id)
        self.assertEqual(correct, str(sqr))

    def test_update_arg_twice(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2, 3, 4)
        sqr.update(4, 3, 2, 89)
        self.assertEqual("[Square] (4) 2/89 - 3", str(sqr))

    def test_update_arg_invalid_size(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            sqr.update(89, "invalid")

    def test_update_arg_zero_size(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            sqr.update(89, 0)

    def test_update_arg_neg_size(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            sqr.update(89, -4)

    def test_update_arg_invalid_x(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            sqr.update(89, 1, "invalid")

    def test_update_arg_x_neg(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "x must be >= 0"):
            sqr.update(98, 1, -4)

    def test_update_arg_invalid_y(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            sqr.update(89, 1, 2, "invalid")

    def test_update_args_y_neg(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "y must be >= 0"):
            sqr.update(98, 1, 2, -4)

    def test_update_arg_size_then_x(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            sqr.update(89, "invalid", "invalid")

    def test_update_arg_size_then_y(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            sqr.update(89, "invalid", 2, "invalid")

    def test_update_arg_x_then_y(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            sqr.update(89, 1, "invalid", "invalid")


class TestSquare_update_kwargs(unittest.TestCase):
    """testing update kwargs method."""

    def test_update_singe_kwarg(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=1)
        self.assertEqual("[Square] (1) 10/10 - 10", str(sqr))

    def test_update_two_kwargs(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(size=1, id=2)
        self.assertEqual("[Square] (2) 10/10 - 1", str(sqr))

    def test_update_three_kwargs(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(y=1, size=3, id=89)
        self.assertEqual("[Square] (89) 10/1 - 3", str(sqr))

    def test_update_four_kwargs(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=89, x=1, y=3, size=4)
        self.assertEqual("[Square] (89) 1/3 - 4", str(sqr))

    def test_update_kwarg_width_set(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=89, size=8)
        self.assertEqual(8, sqr.width)

    def test_update_kwarg_height_set(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=89, size=9)
        self.assertEqual(9, sqr.height)

    def test_update_kwarg_null_id(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=None)
        correct = "[Square] ({}) 10/10 - 10".format(sqr.id)
        self.assertEqual(correct, str(sqr))

    def test_update_kwargs_null_id_more(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=None, size=7, x=18)
        correct = "[Square] ({}) 18/10 - 7".format(sqr.id)
        self.assertEqual(correct, str(sqr))

    def test_update_kwarg_twice(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(id=89, x=1)
        sqr.update(y=3, x=15, size=2)
        self.assertEqual("[Square] (89) 15/3 - 2", str(sqr))

    def test_update_kwarg_invalid_sizze(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "width must be an integer"):
            sqr.update(size="invalid")

    def test_update_kwarg_zero_sizze(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            sqr.update(size=0)

    def test_update_kwarg_size_neg(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "width must be > 0"):
            sqr.update(size=-3)

    def test_update_kwarg_invalid_x(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "x must be an integer"):
            sqr.update(x="invalid")

    def test_update_kwarg_x_neg(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "x must be >= 0"):
            sqr.update(x=-5)

    def test_update_kwarg_invalid_y(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(TypeError, "y must be an integer"):
            sqr.update(y="invalid")

    def test_update_kwarg_y_neg(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaisesRegex(ValueError, "y must be >= 0"):
            sqr.update(y=-5)

    def test_update_arg_and_kwarg(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(89, 2, y=6)
        self.assertEqual("[Square] (89) 10/10 - 2", str(sqr))

    def test_update_kwarg_wrong_key(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(a=5, b=10)
        self.assertEqual("[Square] (10) 10/10 - 10", str(sqr))

    def test_update_kwargs_more_wrong_key(self):
        sqr = Square(10, 10, 10, 10)
        sqr.update(size=5, id=89, a=1, b=54)
        self.assertEqual("[Square] (89) 10/10 - 5", str(sqr))


class TestSquare_to_dictionary(unittest.TestCase):
    """testing to_dictionary method."""

    def test_to_dict_output(self):
        sqr = Square(10, 2, 1, 1)
        correct = {'id': 1, 'x': 2, 'size': 10, 'y': 1}
        self.assertDictEqual(correct, sqr.to_dictionary())

    def test_to_dict_no_obj_change(self):
        sqr1 = Square(10, 2, 1, 2)
        sqr2 = Square(1, 2, 10)
        sqr2.update(**s1.to_dictionary())
        self.assertNotEqual(sqr1, sqr2)

    def test_to_dict_arg(self):
        sqr = Square(10, 10, 10, 10)
        with self.assertRaises(TypeError):
            sqr.to_dictionary(1)

if __name__ == "__main__":
    unittest.main()
